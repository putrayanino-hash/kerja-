...
import { Mail, User as UserIcon, Shield, Link, Key, Lock } from 'lucide-react';
...
export const CreateUserModal: React.FC<CreateUserModalProps> = ({ isOpen, onClose, onSave, userToEdit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Ensure avatar is updated if still using placeholder and name changed
    ...
    // Fallback username/pass if empty
    const finalData = {
        ...formData,
        avatar: finalAvatar,
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={userToEdit ? "Edit Team Member" : "Add Team Member"}>
      <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <input 
                ...
                onChange={e => {
                    setFormData({
                        ...formData, 
                        name: e.target.value,
                        // Update avatar preview dynamically if using default service
        ...
        <div className="pt-4 flex justify-end gap-2 border-t border-slate-100 mt-4">
            ...
            <button 
                type="submit" 
                className="px-6 py-2 bg-primary-600 text-white font-bold rounded-lg hover:bg-primary-700 transition-all shadow-lg shadow-primary-200"
            >
